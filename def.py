...
def get_epg_from_config(config_url):
    """Scarica la configurazione JSON con le informazioni EPG"""
    try:
        response = requests.get(config_url, timeout=10)
        response.raise_for_status()
        return response.json()
    except requests.RequestException as e:
        print(f"Errore durante il download del config JSON da {config_url}: {e}")
        return []

def main():
    # Scarica il config JSON
    config_data = get_epg_from_config(CONFIG_URL)

    # Estrai le informazioni EPG dalla configurazione
